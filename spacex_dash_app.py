...
def get_pie_chart(entered_site):
    spacex_df['class'] = spacex_df['class'].replace({1: 'Success', 0: 'Failure'})
    if entered_site == 'ALL':
        # Count the number of successes per site
        fig = px.pie(spacex_df, names='Launch Site', 
                     title='Total Success Launches By Site')
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        # Count the success and failure cases
        success_count = filtered_df['class'].value_counts().reset_index()
        success_count.columns = ['class', 'count']
        fig = px.pie(success_count, values='count', names='class', 
...
def update_scatter(entered_site,payload_range):
    ...
    fig = px.scatter(filtered_df, x = 'Payload Mass (kg)', y = 'class', color = 'Booster Version Category',
                    title = f'Launch Outcome by Payload Mass {entered_site} site',
